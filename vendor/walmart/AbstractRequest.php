...
abstract class AbstractRequest {
	...
	const BASE_URL_DEV = 'https://sandbox.walmartapis.com';
	...
	public function getHeaders($url, $method, $headers = array()) {
		...
		global $wpdb;
		$sql='SELECT * FROM '.$wpdb->prefix . 'woocommerce_smp_walmart_token'.' client_id id='.$this->config['clientId'];
      	$row = $wpdb->get_row($sql, ARRAY_A);
	
		//$row = \DB::select("SELECT * from wm_token where client_id='".$this->config['clientId']."' ORDER BY id desc limit 1");
		

		if(isset($row[0]))
		{
			//dd($row[0]->access_token);
			$this->config['token'] = array(
			'access_token' => $row[0]->access_token,
			'token_type' => $row[0]->token_type,
			'expires' => $row[0]->expires
			);
		}
		...
		if(isset($this->config['clientId']) && isset($this->config['clientSecret'])) {
			...
			if(!isset($this->config['token']) || $this->config['token']['expires'] <= $requestTime-10) {
				...
				if($response !== false) {
					...
					$wpdb->insert($wpdb->prefix .'woocommerce_smp_walmart_token', array(
			...
			$headers[] = 'WM_QOS.CORRELATION_ID: '.base64_encode($this->generateRandomString());
		} else {
			$time = round(microtime(true)*1000);
